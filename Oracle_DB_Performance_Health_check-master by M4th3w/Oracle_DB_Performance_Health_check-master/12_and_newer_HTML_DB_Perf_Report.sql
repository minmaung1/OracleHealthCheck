/*
  ...
  Author:          Min Maung
  email:           minmaungofficial.mmm@gmail.com
    ...
    cursor c_frag_tables is
      select round((1 - (dt.avg_row_len * dt.num_rows) / (ds.blocks * p.value)) * 100, 2) frag_rate_pct,
       ...
       where dt.owner = u.username
         ...
         and ds.blocks > 10000 -- some reasonable value to filter small tables no one needs to defrag anyway
  ...
  */
  ...
  begin
    ...
    for line in stats_data(p_stat_name) loop
      ...
      queries := queries || '<div id="sql-' || line.sql_id || '-' || to_char(line.child_address) || '" class="hidden">' /*'" class="query-text popup" >'*/ || escape_html_clob(line.sql_fulltext) || '</div>';
    end loop;
